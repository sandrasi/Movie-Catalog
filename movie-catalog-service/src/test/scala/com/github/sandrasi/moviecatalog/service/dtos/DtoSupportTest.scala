...
class DtoSupportTest extends FunSuite with ShouldMatchers {
  ...
  test("should convert actor to actor dto") {
    ...
    val character = domain.Character("Vincent Vega")
    val movie = domain.Movie("Pulp fiction", runtime = Duration.standardMinutes(154), releaseDate = new LocalDate(1994, 10, 14))
    val actor = domain.Actor(person, character, movie)
    implicit val locale = HungarianLocale
    toActorDto(actor) should be(Actor(None, toPersonDto(person), toCharacterDto(character), toMotionPictureDto(movie)))
  }
  ...
  test("should convert actress to actress dto") {
    ...
    val character = domain.Character("Mia Wallace")
    val movie = domain.Movie("Pulp fiction", runtime = Duration.standardMinutes(154), releaseDate = new LocalDate(1994, 10, 14))
    val actress = domain.Actress(person, character, movie)
    implicit val locale = HungarianLocale
    toActressDto(actress) should be(Actress(None, toPersonDto(person), toCharacterDto(character), toMotionPictureDto(movie)))
  }
  ...
  test("should convert charater to character dto") {
    ...
    toCharacterDto(character) should be(Character(None, character.name, character.creator, character.creationDate.toString))
  }
  ...
  test("should convert digital container to digital container dto") {
    ...
    toDigitalContainerDto(digitalContainer) should be(DigitalContainer(None, toMotionPictureDto(movie), digitalContainer.soundtracks.map(toSoundtrackDto(_)), digitalContainer.subtitles.map(toSubtitleDto(_))))
  }
  ...
  test("should convert movie to movie dto") {
    val movie = domain.Movie("Pulp fiction", Set(LocalizedText("Ponyvaregény")(HungarianLocale), LocalizedText("Pulp fiction")(ItalianLocale)), Set(domain.Genre("crime", "Crime"), domain.Genre("thriller", "Thriller")), Duration.standardMinutes(154), new LocalDate(1994, 10, 14))
    toMotionPictureDto(movie)(HungarianLocale) should be(Movie(None, movie.originalTitle.text, Some(movie.localizedTitles.filter(_.locale == HungarianLocale).head.text), movie.genres.map(toGenreDto(_)), movie.runtime.getMillis, movie.releaseDate.toString))
  }
  ...
  test("should convert movie to movie dto if no localized title matches the current locale") {
    ...
    toMotionPictureDto(movie)(HungarianLocale) should be(Movie(None, movie.originalTitle.text, None, movie.genres.map(toGenreDto(_)), movie.runtime.getMillis, movie.releaseDate.toString))
  }
  
  test("should convert person to person dto") {
    val person = domain.Person("John Joseph Travolta", Male, new LocalDate(1954, 2, 18), "Englewood, New Jersey, U.S.")
    toPersonDto(person) should be(Person(None, person.name, person.gender.toString, person.dateOfBirth.toString, person.placeOfBirth))
  }
  ...
  test("should convert soundtrack to soundtrack dto if the language and format names are not defined") {
    ...
    toSoundtrackDto(soundtrack) should be(Soundtrack(None, soundtrack.languageCode, soundtrack.formatCode, None, None))
  }
  ...
  test("should convert subtitle to subtitle dto if the language and format names are not defined") {
    ...
    toSubtitleDto(subtitle) should be(Subtitle(None, subtitle.languageCode, None))
  }
}
